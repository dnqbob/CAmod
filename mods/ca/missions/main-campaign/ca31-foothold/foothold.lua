...
WorldLoaded = function()
	...
	ObjectiveDeploySensorArrays = GDI.AddObjective("Lua-foothold-deploy-sensor-arrays")
	ObjectiveCaptureNerveCenter = GDI.AddObjective("Lua-foothold-capture-scrin-nerve")
	...
	if Difficulty ~= "easy" then
		Trigger.AfterDelay(DateTime.Seconds(10), function()
			...
			Notification("Lua-foothold-dangerous-tiberiumbased-lifeforms")
		end)
	end
	...
	Trigger.OnCapture(NerveCenter1, function(self, captor, oldOwner, newOwner)
		...
		ObjectiveProtectNerveCenter = GDI.AddObjective("Lua-foothold-protect-the-captured")
		ObjectiveDestroyScrinBase = GDI.AddObjective("Lua-foothold-destroy-the-scrin")
		...
		Trigger.AfterDelay(DateTime.Seconds(3), function()
			...
			Notification("Lua-foothold-interstellar-gateway-stabilized")
			GatewayStable = Actor.Create("wormholexl", true, { Owner = GatewayOwner, Location = Gateway.Location })
			Gateway.Destroy()

			Trigger.AfterDelay(DateTime.Seconds(5), function()
				PlaySpeechNotificationToMissionPlayers("ReinforcementsArrived")
				Notification("Lua-foothold-reinforcements-have-arrived")
				...
				Trigger.AfterDelay(AdjustTimeForGameSpeed(DateTime.Seconds(4)), function()
					...
					Notification("Lua-foothold-do-not-allow")
				end)
			end)
		end)
	end)
...
CheckSensors = function()
	...
	if NumSensorsDeployed == 4 then
		...
		Trigger.AfterDelay(DateTime.Seconds(2), function()
			...
			Notification("Lua-foothold-nerve-center-located")
			...
			Trigger.AfterDelay(DateTime.Seconds(9), function()
				if not SPower2.IsDead then
					...
					Notification("Lua-foothold-the-nerve-center")
		...
		UserInterface.SetMissionText("Lua-foothold-sensor-arrays-deployed" .. NumSensorsDeployed .. "/4", HSLColor.Yellow)
	end
end
